...
class Dataset:
    def __init__(self, batch_size, folder='CamSeq01'):
    ...
    def file_paths_to_images(self, folder, files_list, verbose=False):
        ...
        for file in files_list:
            ...
            input_image_array = cv2.imread(input_image, 1)
            height, width, channel = input_image_array.shape[:3]
            target_image_array = cv2.imread(input_image, 1)

            if input_image_array is None or target_image_array is None:
                print("Failed to load image file.")
                sys.exit(1)
            else:
                inputs.append(input_image_array)
                targets.append(target_image_array)
                heights.append(height)
                widths.append(width)
                channels.append(channel)

            print("input_image_array:" + input_image_array)
            print("target_image_array:" + target_image_array)
            # カラーとグレースケールで場合分け
            #if len(input_image_array.shape) == 3:
            #    height, width, channels = input_image_array.shape[:3]
            #else:
            #    height, width = input_image_array.shape[:2]
            #    channels = 1
            ...
            # load grayscale: test_image = np.array(cv2.imread(input_image, 0))
        ...
        return inputs, targets, heights, widths, channels
    ...
    def label_names_colors(self):
        file = open('./CamSeq01/label_colors.txt', 'r')  # 読み込みモードでオープン
        string = file.read()  # readですべて読み込む
        words = string.split() #単語ごとに分割
        label_names = []
        label_colors = []

        for i in range(int(len(words) / 4)):
            label_colors.append([int(words[i * 4]), int(words[i * 4 + 1]), int(words[i * 4 + 2])])
            label_names.append(words[i * 4 + 3])
...
def train():
    ...
    dataset = Dataset(folder='CamSeq01', batch_size=BATCH_SIZE)

    inputs, targets, heights, widths, channels = dataset.file_paths_to_images()#####
    ...
    with tf.Session() as sess:
        ...
        for epoch_i in range(n_epochs):
            ...
            for batch_i in range(dataset.num_batches_in_epoch()):
                ...
                print('{}/{}, epoch: {}, cost: {}, batch time: {}, start time: {}'.format(batch_num,
                                                                          n_epochs * dataset.num_batches_in_epoch(),
                                                                          epoch_i, cost, end - start, start_time))
