...
class Test:
    def __init__(self, folder, resized_folder, segmented_folder): #(self, folder="DLimgs", resized_folder="streetview", segmented_folder="segmented")
        print("##############################################################################")
    ...
    def resize_images(self, streetview, i, resized_folder):
        ...
        if (inh != inw):
            ...
            print("creating resized streetview: ")
    ...
    def segmentationStreetview(self, streetviewName, network, resized_folder, segmented_folder):
        ...
        "C:/Users/yuriK/OneDrive/ドキュメント/#colour/code/Tensorflow-Segmentation-master/Tensorflow-Segmentation-master/save/C7,64,2C7,64,1M2C7,64,2C7,64,1M2C7,64,2C7,64,1M2/2018-12-02_131913"
        ...
        with tf.Session() as sess:
            ...
            accuracy = 0
            for res_num in range(len(inputs)):

                input_path = os.path.join(folder_path, "inputs", inputs[res_num])
                target_path = os.path.join(folder_path, "targets", targets[res_num])
                test_data = np.array(cv2.imread(input_path, 1))
                test_label = np.array(cv2.imread(target_path, 1))

                test_data = np.reshape(test_data,
                                         (-1, network.IMAGE_HEIGHT, network.IMAGE_WIDTH, network.IMAGE_CHANNELS))
                test_label = np.reshape(test_label,
                                          (-1, network.IMAGE_HEIGHT, network.IMAGE_WIDTH, network.IMAGE_CHANNELS))

                # test_inputs = np.multiply(test_inputs, 1.0 / 255)
                test_data = np.multiply(test_data, 1.0 / 255)
                test_label = np.multiply(test_label, 1.0 / 255)
                ...
                res = sess.run(network.accuracy, feed_dict={
                    network.inputs: test_data,
                    network.targets: test_label,
